...
def generate_images(
    ...
    if not resolved_image_paths:
        raise ValueError("At least one product image is required for batch image generation.")
    ...
    base_prompt_template = prompt_text.strip() if prompt_text else load_prompt(prompt_file)
    base_prompt = (
    ...
    for index in range(1, max(1, num_images) + 1):
        ...
        content_items: list[dict[str, Any]] = [{"type": "input_text", "text": request_text}]
        for resolved in resolved_image_paths[:4]:
            content_items.append(
                {
                    "type": "input_image",
                    "image_url": _to_data_url(Path(resolved)),
                    "detail": "low",
                }
            )
    ...
    with tempfile.TemporaryDirectory(prefix="openai_batch_") as temp_dir:
        ...
        while True:
            ...
            if status in {"failed", "expired", "cancelled"}:
                raise RuntimeError(f"OpenAI batch failed with status={status} (batch_id={current.id})")
        ...
        if not output_file_id:
            raise RuntimeError(f"OpenAI batch completed without output_file_id (batch_id={batch.id})")
