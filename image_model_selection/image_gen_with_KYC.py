...
def build_logger(job_log_file: str | None) -> JsonLogger:
    if job_log_file:
        return JsonLogger(
            global_log_file=str(PROJECT_ROOT / "storage/logs/execution.log"),
            job_log_file=job_log_file,
            include_global_when_job_active=True,
        )
    return JsonLogger(global_log_file=str(PROJECT_ROOT / "storage/logs/execution.log"))
...
def generate_images(
    ...
    """
    stage_logger = logger_obj or build_logger(None)
...
def run_default() -> int:
    ...
    generate_images(
        image_path=str(project_root / "tatsya_product.jpg"),
        brand_name="tatsya",
        kyc_path=str(script_dir / "tatsya_tatsya_product_kyc.json"),
        num_images=4,
    )
    return 0
