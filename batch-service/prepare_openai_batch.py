...
PRODUCT_CATEGORY_PROMPT_FILES = {
    "pets_accessories": "pets_accessories.txt",
    "cushions": "cushions.txt",
    "bathmat": "bathmat.txt",
    "frozen_food": "frozen.txt",
}
...
def normalize_product_category(value: str) -> str:
    ...
    aliases = {
        ...
        "pet_accessories": "pets_accessories",
        "petsaccessories": "pets_accessories",
        "cushions": "cushions",
        "cushion": "cushions",
        "bathmat": "bathmat",
        "bathmats": "bathmat",
    ...
    return aliases.get(cleaned, "pets_accessories")


def load_product_category_prompt(category: str) -> str:
    normalized = normalize_product_category(category)
    filename = PRODUCT_CATEGORY_PROMPT_FILES.get(normalized, PRODUCT_CATEGORY_PROMPT_FILES["pets_accessories"])
    prompt_path = PROMPTS_DIR / filename
    if not prompt_path.exists():
        raise FileNotFoundError(f"Prompt file not found for category '{normalized}': {prompt_path}")
...
def build_flat_image_rows(
    ...
    scene_brief: str,
    *,
    product_category: str = "pets_accessories",
...
def build_manifest_rows_from_input_root(
    input_root: Path,
    scene_brief: str,
    product_category: str = "pets_accessories",
) -> list[dict[str, str]]:
...
DEFAULT_CATEGORY_SHOT_VARIANTS = {
    "pets_accessories": ["closeup", "angle", "dog"],
    "cushions": ["closeup", "angle", "styled"],
    "bathmat": ["closeup", "angle", "room"],
    "frozen_food": ["packshot", "angle", "serving_scene"],
}


def parse_shot_variants(category: str) -> list[str]:
    normalized = normalize_product_category(category)
    default_variants = DEFAULT_CATEGORY_SHOT_VARIANTS.get(normalized, DEFAULT_CATEGORY_SHOT_VARIANTS["pets_accessories"])
    raw_value = env_or_default("OPENAI_BATCH_SHOT_VARIANTS", ",".join(default_variants)).lower()
    requested = [item.strip() for item in raw_value.split(",") if item.strip()]
    allowed = {"closeup", "angle", "dog", "styled", "room", "packshot", "serving_scene"}
    selected = [item for item in requested if item in allowed]
    return selected or list(default_variants)


def shot_instruction(category: str, shot_variant: str) -> str:
    normalized = normalize_product_category(category)
    if shot_variant == "packshot":
        return (
            "SHOT TYPE: premium food packshot. Create a clean premium ecommerce packshot for a frozen food product. "
            "Keep the packaging fully visible, front-facing or slightly angled, easy to read, and isolated in a polished commercial setup. "
            "Do not include people or pets."
        )
    if shot_variant == "closeup":
        return (
            "SHOT TYPE: close-up product detail. Create a premium close-up ecommerce shot that highlights "
            "fabric, stitching, shape, and texture. Keep the full product visible in frame. "
            "Do not include pets or people unless explicitly requested."
        )
    if shot_variant == "angle":
        return (
            "SHOT TYPE: 3/4 angle product view. Show the product from a realistic three-quarter angle, emphasizing "
            "depth, structure, and form. Keep the full product visible with margin. "
            "Do not include pets or people unless explicitly requested."
        )
    if shot_variant == "styled":
        return (
            "SHOT TYPE: styled interior lifestyle view. Place the product in a tasteful premium home setting that suits "
            "a cushion product. Keep the full product clearly visible and make the environment supportive, not distracting."
        )
    if shot_variant == "room":
        return (
            "SHOT TYPE: premium room-context view. Place the product in a realistic premium bathroom or spa-like setting "
            "appropriate for a bath mat. Keep the full product clearly visible and easy to read."
        )
    if shot_variant == "serving_scene":
        return (
            "SHOT TYPE: serving scene. Show the frozen food product in a realistic premium serving context with appetizing plated or prepared presentation "
            "that matches the packaging claim. Keep the product packaging visible in frame and make the food look natural, high quality, and commercially styled."
        )
    if normalized != "pets_accessories":
        return (
            "SHOT TYPE: premium lifestyle context shot. Keep the full product visible in a category-appropriate setting. "
            "Do not include pets or people unless explicitly requested."
        )
    return (
        "SHOT TYPE: real dog lifestyle shot. Include exactly one realistic dog model. If the product is wearable "
        "(cap / harness / bandana), the dog must wear it naturally. If the product is non-wearable (toy / bed), "
        "the dog must interact with it naturally. The product must remain fully visible and the dog must not block "
        "edges, shape, branding, or structure."
    )


def build_prompt(row: dict[str, str], shot_variant: str, product_kyc: dict[str, Any] | None) -> str:
    product_name = row.get("product_name", "").strip() or "product"
    category = normalize_product_category(row.get("product_category", ""))
    ...
    parts = [
        load_product_category_prompt(category),
        f"Product name: {product_name}.",
        f"Category: {category}.",
        shot_instruction(category, shot_variant),
...
def make_batch_request(
    *,
    ...
    prompt = build_prompt(row, shot_variant, product_kyc)
    ...
    return {
        "custom_id": f"{product_id}__{shot_variant}",
        "method": "POST",
        "url": "/v1/responses",
        "body": {
            "model": batch_model,
...
def prepare_batch_with_options(
    ...
    *,
    ...
    for row in rows:
        category = normalize_product_category(row.get("product_category", ""))
        shot_variants = parse_shot_variants(category)
        selected_image_count = max(1, min(int(image_count or len(shot_variants)), len(shot_variants)))
        shot_variants = shot_variants[:selected_image_count]
        ...
        for shot_variant in shot_variants:
...
def get_batch_status(batch_id: str) -> dict[str, object]:
    ...
    if error_file_id and not output_file_id and str(batch.status) in {"completed", "failed", "cancelled", "expired"}:
        errors = []
        try:
            error_text = read_file_content_text(client, error_file_id)
            for line in error_text.splitlines():
                ...
                row = json.loads(line)
                response_block = row.get("response", {})
                body = response_block.get("body", {}) if isinstance(response_block, dict) else {}
                error_block = body.get("error", {}) if isinstance(body, dict) else {}
                errors.append(
                    {
            ...
            errors = [{"message": f"Unable to read error file: {exc}"}]
