...
REFERENCE_IMAGES_FOLDER = "/home/anhad/edxso/contentpro/spunkies"
...
def _discover_jobs(root_folder_path: str, logger: logging.Logger) -> list[dict[str, Any]]:
    root_folder = Path(root_folder_path).expanduser().resolve()
    if not root_folder.exists() or not root_folder.is_dir():
        raise ValueError(f"REFERENCE_IMAGES_FOLDER is invalid: {root_folder}")

    job_folders = sorted(path for path in root_folder.iterdir() if path.is_dir())
    if not job_folders:
        raise ValueError(f"No job subfolders found in {root_folder}")
    ...
    for job_folder in job_folders:
        ...
        if len(images) < 3:
            logger.warning(
                "Skipping job folder '%s' because it has fewer than 3 reference images (found=%s).",
                job_folder.name,
                len(images),
            )
            continue
        ...
        if len(images) > 3:
            logger.info(
                "Job '%s' has %s images; using first 3 only.",
                job_folder.name,
                len(images),
            )
        jobs.append(
    ...
    if not jobs:
        raise ValueError(
            "No valid jobs found. Each subfolder must contain at least 3 reference images."
        )
...
def run() -> dict[str, Any]:
    ...
    jobs = _discover_jobs(REFERENCE_IMAGES_FOLDER, logger)
    logger.info("Discovered %s jobs from subfolders.", len(jobs))

    prompt_text = _build_prompt(PROMPT, SHOT_PROMPTS)
    requests: list[dict[str, Any]] = []
    custom_id_to_job: dict[str, str] = {}
    for index, job in enumerate(jobs, start=1):
        job_name = str(job["job_name"])
        custom_id = f"job_{index:03d}_{_slugify(job_name)}"
        custom_id_to_job[custom_id] = job_name
        requests.append(
            _make_request(custom_id, prompt_text, job["reference_images"])
        )
    ...
    for request in requests:
        ...
        job_name = custom_id_to_job.get(custom_id)
        selected_job = next((job for job in jobs if job["job_name"] == job_name), None)
        preview_rows.append(
            {
                ...
                "source_reference_images": [
                    str(path) for path in (selected_job["reference_images"] if selected_job else [])
                ],
    ...
    saved_images_by_job: dict[str, list[str]] = {str(job["job_name"]): [] for job in jobs}
    for entry in output_entries:
        if not isinstance(entry, dict):
            continue
        custom_id = str(entry.get("custom_id") or "unknown")
        job_name = custom_id_to_job.get(custom_id, "unknown_job")
        job_output_dir = outputs_dir / job_name
        job_output_dir.mkdir(parents=True, exist_ok=True)
        error = entry.get("error")
        if error:
            logger.error("Request error for custom_id=%s: %s", entry.get("custom_id"), error)
            continue
        job_saved = _save_generated_image(entry, job_output_dir, logger)
        saved_images.extend(job_saved)
        if job_name not in saved_images_by_job:
            saved_images_by_job[job_name] = []
        saved_images_by_job[job_name].extend(job_saved)
    ...
    summary = {
        ...
        "saved_images_by_job": saved_images_by_job,
        "jobs": [
            {
                "job_name": str(job["job_name"]),
