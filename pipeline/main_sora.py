...
def main() -> None:
    parser = argparse.ArgumentParser(description="Orchestrate the 4-stage image-to-video pipeline (Sora 2 edition).")
    ...
    try:
        ...
        for idx, png_path in enumerate(image_files, start=1):
            ...
            rc4, stdout4, _ = run_stage_subprocess(
                script_name="video_gen_sora.py",
                stage_name="stage_4_video_gen_sora",
                script_args=[
                    "--image-path",
                    str(png_path),
                    "--kyc-path",
            ...
            video_path = ensure_result_path(
                stage4_result["video_path"], "stage_4_video_gen_sora", "video_path"
            )
